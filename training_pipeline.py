...
from metaflow import Config, FlowSpec, IncludeFile, Parameter, card, resources, kubernetes, pypi, step, checkpoint, model, current
...
class PIITrainingPipeline(FlowSpec):
    ...
    def start(self):
        ...
        from model.src.config import EnvironmentSetup, TrainingConfig
    ...
    @kubernetes(memory=16000, cpu=8)  # Run on K8s to avoid local onnxruntime issues
    @step
    def quantize_model(self):
        ...
        from model.src.quantitize import (
            export_to_onnx,
    ...
    def sign_model(self):
        ...
        try:
            ...
            if self.quantized_model is not None:
                print("Loading quantized model checkpoint for signing...")
                checkpoint_ref = self.quantized_model
                model_type = "quantized"
            else:
                print("Loading trained model checkpoint for signing...")
                checkpoint_ref = self.trained_model
                model_type = "trained"

            # Use Checkpoint as context manager to load files to a directory
            with Checkpoint() as cp:
                cp.load(checkpoint_ref)
                model_to_sign = cp.directory
                print(f"  Loaded {model_type} model from checkpoint to: {model_to_sign}")
